...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="The number of  to use for the preprocessing.",
    )
    parser.add_argument(
        "--overwrite_cache", action="store_true", help="Overwrite the cached training and evaluation sets"
    )
    parser.add_argument(
        "--fp16", action="store_true", help="Whether or not to use 16-bit (mixed) precision (through NVIDIA apex) instead of 32-bit"
    )
    parser.add_argument(
...
def main():
    args = parse_args()
    with open('data/amino.txt', 'r') as f:
        animo_data = f.readlines()
    animo_data = ['<'+i.strip()+'>' for i in animo_data]
    
    special_tokens_dict = {'additional_special_tokens': animo_data}
    ...
    if args.model_name_or_path == 'Rostlab/prot_bert':
    ...
    if args.line_by_line:
        ...
        # tokenized_datasets = torch.load("tokenized_datasets.pt")
    ...
    # tokenized_datasets.save_to_disk('/m-ent1/ent1/smap20/data/tokenized_dataset')
